...
初次结果为 420 项通过。持续故障注入、可变返回值和并发审查后，最终结果为 446 项
通过，13 项真实端点测试按既有条件跳过；墙钟 70.87 秒；最大 RSS 206,684,160 bytes。
...
产业链初次离线全仓回归为 630 项通过。加入写入器矩阵、失败通知、并发压力、性能索引和
跨资源恢复测试后，最终为 670 项通过、17 项按既有条件跳过；墙钟 41.69 秒；最大 RSS
331,546,624 bytes。
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 普通 dict | 4.414 秒 | 123,944,960 | 245,760 | 无 | 无 | 无 |
| `transparent_proxy` | 9.296 秒 | 163,954,688 | 40,157,184 | 14,705 | 22,183 | 36,888 |
| `explicit_transactions` | 18.831 秒 | 194,691,072 | 70,451,200 | 14,672 | 22,183 | 36,855 |

显式模式把同一事务内对同一记录的多次字段写合并，所以 replacement 比代理少 33 条，追加
事实数量相同。第二轮先用 profile 找到两类真实热点：核心层的重复 overlay 查找、重复
journal 候选和 canonical touched-list 线性查重；领域层的市场编译器曾对每个产品重复扫描、
解析整张采购请求表。核心改为只处理最终 replacement 候选，并为未改状态根设置读取快路径；
领域层改为每个自然日构建一次按产品分组的采购请求索引，并在一次采购请求刷新内部按完整
输入复用完全相同的生产可行性查询；该复用不跨自然日，也不跨市场或生产写入边界。相对
第二轮优化前的同 checkpoint 结果，显式模式从 49.318 秒降到 18.831 秒（下降 61.8%），
代理从 13.829 秒降到 9.296 秒（下降 32.8%），普通 dict 从 7.517 秒降到 4.414 秒
（下降 41.3%）。逐根状态 hash、追加事实数量和最终经济状态摘要保持一致。

当前显式模式仍是代理的 2.03 倍、普通 dict 的 4.27 倍。剩余主成本来自“一整个自然日一个
事务”造成的数百万次通用 overlay 读取，以及提交时必须保留的有界 COW、终态校验和冲突
检查。合成写入中，把 5,000 次写聚合为一个事务时显式模式约 117 ms、代理约 126 ms；若
每次写各开一个事务，显式模式约 807 ms。事务边界和领域读路径决定了实际速度，不能只靠
删掉校验获得接近普通字典的性能。一次跨生产日复用 feasibility 结果的候选优化虽然更快，
但改变了 7 日状态 hash 和 delta，已完整回退，没有进入本分支。
...
| --- | ---: | ---: | ---: | ---: |
| `transparent_proxy` | 211,687 ns | 639,354 ns | 6 / 1,743 bytes | 1,409,024 bytes |
| `explicit_transactions` | 295,000 ns | 662,375 ns | 6 / 1,743 bytes | 1,998,848 bytes |
...
- 共享记录并发使用乐观版本检查和串行 commit：两个 asyncio task 修改同一记录时一方提交、
  一方得到 `StateTransactionConflict`；新事务重试成功。冲突基线固定在事务进入时，因此调用方
  先读、等待、再写，也不能用旧读值覆盖等待期间的提交。不同记录、不同 append ID 可以并行；
  append list 按 commit 顺序落盘。同一冲突文件连续重复 20 次稳定通过，产业链另用固定种子跑了
  100 轮交错压力，没有出现 lost update 或失败前缀。
- 真实 Industry Action 通过两个 Agent 并发向同一主体发送直接消息，强制两笔事务都完成 staging
  后再 commit。结果一笔成功，一笔得到 `StateTransactionConflict`；失败方没有留下 DM、
  Conversation 或 Inbox 前缀。两个 Agent Thread 都正常关闭，失败 Thread 记录明确错误，V4
  marker 仍完整发布。通用 Action 层没有自动重试，因为整段 Action 可能包含 World 之外的副作用；
  调用方可在确认业务幂等后显式开启新事务重试。
- 审查还发现并修复了四类可变视图边界：事务根/子树 `deepcopy()` 现在合成全部读己写；
  `list.pop()` 和 `dict.popitem()` 返回脱离 canonical 的普通值；事务期间禁止从另一 context 整体
  替换 Environment.state；未完成最终校验的 journal token 不能直接提交。相应测试同时验证
  commit、rollback、mixed entity 和 append-only 拒绝语义。

## 残余风险

- 显式整日事务虽已提速 61.8%，仍比代理慢 2.03 倍；默认模式继续是
  `transparent_proxy`。若要继续逼近普通字典，需要把产业链热点 writer 改为记录级暂存接口，
  减少通用 Mapping 视图上的百万次读取，同时保留 COW、终态校验和冲突检测。
