# libs/simengine/src/simengine/decorators.py
"""
装饰器系统（v3.0 - 完全重写）

本模块提供两套装饰器：
1. 内置装饰器（用于 simengine 内部）：@env_type, @capability
        ...
        for param_name, param in params.items():
            ...
            if annotation != inspect.Parameter.empty:
                ...
                if 'ExecutionContext' in annotation_str or param_name == 'context':
                    context_param = {
                        'type': annotation_str if 'ExecutionContext' in annotation_str else 'simengine.core_data.ExecutionContext',
                        'name': param_name
                    }
