...
class Event:  # pragma: no cover
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        raise RuntimeError(
            "simengine.event_logger.Event 已停用，请迁移到 simengine.events 中的事件模型。"
        )
...
class EventLogger:  # pragma: no cover
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        raise RuntimeError(
            "simengine.event_logger.EventLogger 已停用，请使用 simengine.transaction.EventLogger "
            "或 simengine.logging.ExperimentLogContext。"
        )
