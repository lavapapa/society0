...
class EmbeddingManager:
    ...
    async def request(self, texts: List[str], dimensions: int = 512) -> Dict[str, Any]:
        """
        统一的Embedding请求接口
        ...
        if not texts:
            return {"result": [], "model": endpoint.model, "dimensions": dimensions}

        model = endpoint.model
        bucket_key = self._make_microbatch_bucket_key(model, dimensions)
        ...
        async with self._cache_lock:
            for idx, text in enumerate(texts):
                cache_key = self._make_cache_key(model, dimensions, text)
        ...
        return {"result": final_results, "model": model, "dimensions": dimensions}
