...
class StepFlow:
    ...
    def _compile_operator(self, operator_config: Dict[str, Any]) -> tuple[Operator, Dict[str, Any]]:
        ...
        operator_name_keys = {'type', 'name', 'rule_name', 'behavior_name'}
        params = {k: v for k, v in operator_config.items() if k not in operator_name_keys}
        ...
        if operator_type == 'instruct':
            ...
            rule_name = operator_config.get('name') or operator_config.get('rule_name')
            if rule_name:
            ...
            behavior_name = operator_config.get('name') or operator_config.get('behavior_name')
            if behavior_name:
    ...
    def _create_rule_operator(self, rule_name: str) -> Operator:
        ...
        async def rule_operator(agents: List['Agent'], params: Dict[str, Any], context: ExecutionContext) -> BaseOperatorResult:
            ...
            signature_params = [
                name for name in rule_sig.parameters
                if name not in {'self', 'cls'}
            ]

            # Legacy FunctionRegistry rules used positional signatures:
            #   agent rule: async def rule(agent, world, params)
            #   env rule:   async def rule(environment, world, params)
            # Keep YAML/studio schedules compatible while newer rules continue
            # through the schema-driven mapping below.
            if (
                'world' in rule_sig.parameters
                and 'params' in rule_sig.parameters
                and signature_params
                and signature_params[0] in {'agent', 'environment', 'env'}
            ):
                try:
                    import time
                    start_time = time.time()

                    if signature_params[0] == 'agent':
                        per_agent_results = {}
                        for agent in agents:
                            per_agent_results[agent.id] = await invoke_maybe_async(
                                rule_func,
                                agent,
                                context.world,
                                params,
                        ...
                        value = (
                            next(iter(per_agent_results.values()))
                            if len(per_agent_results) == 1
                            else per_agent_results
                        )
                    else:
                        env = context.world.get_environment()
                        value = await invoke_maybe_async(rule_func, env, context.world, params)

                    execution_time = time.time() - start_time
                    context.log_event("rule_execution", source="schedule.rule_operator", data={
                        "rule_name": rule_name,
                        "legacy_signature": True,
                        "execution_time": execution_time,
                    })
                    return BaseOperatorResult(
                        agent_id="batch" if len(agents) > 1 else (agents[0].id if agents else "environment"),
                        status="success",
                        value=value,
                        execution_time=execution_time,
                        metadata={
                    ...
                    error_msg = f"Error executing legacy rule '{rule_name}': {str(e)}"
                    logger.error(error_msg)
                    return BaseOperatorResult(
                        agent_id="environment",
                        status="error",
