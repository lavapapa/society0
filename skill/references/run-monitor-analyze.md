...
- `agent_batch_started` / `agent_batch_heartbeat` / `agent_batch_progress` / `agent_batch_completed` events for each `instruct` or `interview`: agent count, concurrency, started count, in-flight count, pending count, completed count, duration, success count, and error count. After the run, prefer `summary.json -> events.agent_batches` for the compact roll-up.
